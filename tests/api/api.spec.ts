...
async function logResponse(resp: APIResponse, label: string) {
  ...
  try {
    const req = (
      resp as unknown as { request?: () => { method(): string; url(): string } }
    ).request?.();
  ...
  console.error('Selected headers:', {
...
test('register -> login -> notes CRUD', async ({ apiCall, apiConfig }) => {
  ...
  try {
    ...
    const create = await apiCall('post', 'notes', {
      data: notePayload,
    ...
    // Get note by id
    const getOne = await apiCall('get', `notes/${noteId}`, {
      headers: { [apiConfig.http.authHeader]: token! },
    });
    expect(getOne.status()).toBe(200);
    const singleBody = await getOne.json();
    const singleNote =
      singleBody && singleBody.title
        ? singleBody
        : (singleBody && (singleBody.data || singleBody.item)) || {};
    expect(singleNote.title).toBe(notePayload.title);

    // Update the note (PATCH used instead of PUT for compatibility)
    const updatedPayload = {
      title: 'API note - updated',
      description: 'Updated description',
      category: 'Home',
    };
    const update = await apiCall('patch', `notes/${noteId}`, {
      data: updatedPayload,
      headers: { [apiConfig.http.authHeader]: token! },
    });
    ...
    const patch = await apiCall('patch', `notes/${noteId}`, {
      data: { completed: true },
      headers: { [apiConfig.http.authHeader]: token! },
    });
    ...
    // Delete the note
    const del = await apiCall('delete', `notes/${noteId}`, {
      headers: { [apiConfig.http.authHeader]: token! },
    });
    ...
    if (token) {
      const cleanup = await apiCall('delete', apiConfig.endpoints.deleteAccount, {
        headers: { [apiConfig.http.authHeader]: token },
      });
      if (!cleanup.ok()) {
